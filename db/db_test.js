...
function testSave() {
  // 创建UserModel的实例
  const userModel = new UserModel({username: 'tom', password: md5('234'), type: 'laoban'})
  // 调用save()保存
...
function testFind() {
  ...
  UserModel.find({_id:'5d400fae450810065cd8d033'}, function (error, users) {
    console.log('find()', error, users)
  })
  // 查询一个: 得到是匹配的文档对象, 如果没有匹配的就是null
  UserModel.findOne({_id:'5d400fae450810065cd8d033'}, function (error, user) {
...
// 3.3. 通过Model的findByIdAndUpdate()更新某个数据
function testUpdate() {
  UserModel.findByIdAndUpdate({_id:'5d400fae450810065cd8d033'},
    {username:'liming'}, function (error, oldUser) {
...
// 3.4. 通过Model的remove()删除匹配的数据
function testDelete() {
  UserModel.remove({_id:'5d400fae450810065cd8d033'}, function (error, doc) {
